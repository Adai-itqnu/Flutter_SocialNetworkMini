...
class FirestoreService {
  ...
  Future<void> updateUserRole(String uid, String role) =>
      _userService.updateUserRole(uid, role);

  Future<List<UserModel>> getAllUsers({int limit = 50}) =>
      _userService.getAllUsers(limit: limit);
  ...
  Future<bool> hasLikedPost(String postId, String userId) =>
      _postService.hasLikedPost(postId, userId);
  ...
  Future<void> savePost(String userId, String postId) =>
      _postService.savePost(userId, postId);

  Future<void> unsavePost(String userId, String postId) =>
      _postService.unsavePost(userId, postId);

  Future<bool> hasSavedPost(String userId, String postId) =>
      _postService.hasSavedPost(userId, postId);

  Future<List<PostModel>> getSavedPosts(String userId) =>
      _postService.getSavedPosts(userId);

  Future<Set<String>> getSavedPostIds(String userId) =>
      _postService.getSavedPostIds(userId);
  ...
  Future<bool> hasLikedComment(String commentId, String userId) =>
      _commentService.hasLikedComment(commentId, userId);

  Stream<List<CommentModel>> getReplies(String parentCommentId) =>
      _commentService.getReplies(parentCommentId);

  // ==================== FOLLOW OPERATIONS ====================

  Future<void> followUser(String followerId, String followingId) =>
      _followService.followUser(followerId, followingId);

  Future<void> unfollowUser(String followerId, String followingId) =>
      _followService.unfollowUser(followerId, followingId);

  Future<bool> isFollowing(String followerId, String followingId) =>
      _followService.isFollowing(followerId, followingId);
